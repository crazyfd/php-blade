...
class Blade implements FactoryContract
{
    ...
    public function __construct($viewPaths, string $cachePath, ?ContainerInterface $container = null, array $options = [])
    {
        $this->container = $container ?: new Application;

        $this->setupContainer((array) $viewPaths, $cachePath, $options);
     ...
     * @param  array<string, mixed>  $options  extra config keys, e.g. view.cache / view.compiled_extension / view.check_timestamps
     * @return void
     */
    protected function setupContainer(array $viewPaths, string $cachePath, array $options = [])
    {
        ...
        $this->container->bindIf('config', function () use ($viewPaths, $cachePath, $options) {
